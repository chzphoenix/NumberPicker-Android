package com.hcz.fastwidget.numberpicker;
...
public class NumberPicker extends LinearLayout {
    ...
    private int selector_whell_middle_item_index = selector_wheel_item_count / 2;
    ...
    public NumberPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        mSolidColor = attributesArray.getColor(
                R.styleable.NumberPicker_android_solidColor, 0);
        mSelectionDivider = attributesArray
                .getDrawable(R.styleable.NumberPicker_numberpickerDivider);
        ...
        mSelectionDividerHeight = attributesArray.getDimensionPixelSize(
                R.styleable.NumberPicker_numberpickerDividerHeight,
        ...
        selector_wheel_item_count = attributesArray.getInt(
                R.styleable.NumberPicker_numberpickerRows, selector_wheel_item_count);
        if(selector_wheel_item_count % 2 == 0){
            throw new IllegalArgumentException("numberpickerRows must is odd");
        }
        selector_whell_middle_item_index = selector_wheel_item_count / 2;
        mSelectorIndices = new int[selector_wheel_item_count];
        int textSize = attributesArray.getDimensionPixelSize(
                R.styleable.NumberPicker_android_textSize, 0);
        int textColor = attributesArray.getColor(
                R.styleable.NumberPicker_android_textColor, 0);
        editable = attributesArray.getBoolean(
                R.styleable.NumberPicker_numberpickerEditable, false);
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        if (changed) {
            ...
            mTopSelectionDividerTop = (getHeight() - getHeight() / selector_wheel_item_count)
                    / 2 - mSelectionDividerHeight;
            mBottomSelectionDividerBottom = mTopSelectionDividerTop + 2
                    * mSelectionDividerHeight + getHeight() / selector_wheel_item_count;
        }
    }
    @Override
    ...
    public void scrollBy(int x, int y) {
        ...
        while (mCurrentScrollOffset > mSelectorElementHeight + mInitialScrollOffset - 10) {
            ...
            setValueInternal(
                    selectorIndices[selector_whell_middle_item_index],
                    true);
            if (!mWrapSelectorWheel
                    && selectorIndices[selector_whell_middle_item_index] <= mMinValue) {
                mCurrentScrollOffset = mInitialScrollOffset;
